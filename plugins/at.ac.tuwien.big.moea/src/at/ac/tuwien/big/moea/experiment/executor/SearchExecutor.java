...
public class SearchExecutor extends Executor {
	...
	public SearchExecutor withProblem(ISearchProblem<?> problem) {
		if (problem instanceof IParetoFrontStoringSearchProblem) {
			IParetoFrontStoringSearchProblem<?> mainprobl = (IParetoFrontStoringSearchProblem)problem;
	...
	protected int getNumberOfThreads() {
		return 1;//getField(THREADS_NAME, Integer.class, 1);
	}
	...
	protected NondominatedPopulation runAlgorithm(Problem problem, int maxEvaluations) {
		...
		try {
			...
			while (!algorithm.isTerminated() && 
				...
				newPop.addAll(intermediateResult);
			...
			NondominatedPopulation pop = algorithm.getResult();
			System.out.println("Solutions found:");
			for (Solution o: pop) {
				System.out.println(Arrays.toString(o.getObjectives()));
			}
	...
	public NondominatedPopulation run() {
		...
		long[] arr = new long[]{curDate,curDate};
		times.add(arr);
