...
public abstract class AbstractLocalSearchAlgorithm<S extends Solution> extends AbstractAlgorithm implements LocalSearchAlgorithm<S>, RestartableAlgorithm<S> {
	...
	protected void setCurrentSolution(S solution) {
		this.currentSolution = (S)solution.copy();
		this.currentFitness = getValue(this.currentSolution);
		bestValues.add(this.currentSolution);
	}
	...
	protected void setBestSolution(S bestSolution) {
		this.bestSolution = (S)bestSolution.copy();
		this.bestFitness = getValue(this.bestSolution);
		bestValues.add(this.bestSolution);
	}
	...
	protected void initialize() {
		...
		setBestSolution((S)getCurrentSolution().copy());
	}
	...
	protected INeighborhood<S> generateCurrentNeighbors() {
		...
		return getNeighborhoodFunction().generateNeighbors((S)getCurrentSolution().copy(),forbidden);
	}
	...
	protected INeighborhood<S> generateCurrentNeighbors(int maxNeighbors) {
		...
		/*if(getCurrentSolution().getNumberOfVariables() >= getProblem().getNumberOfVariables())
			return getNeighborhoodFunction().createEmptyNeighborhood();*/
		return getNeighborhoodFunction().generateNeighbors((S)getCurrentSolution().copy(), maxNeighbors, forbidden);
	}
	...
	public void copyFor(AbstractLocalSearchAlgorithm<S> algorithm, S initialSolution) {
		this.initialSolution = (S)initialSolution.copy();
	...
	protected abstract AbstractLocalSearchAlgorithm<S> newInstance(Problem problem);
	
	
	
	public AbstractLocalSearchAlgorithm<S> prototypeFor(S s, IFitnessComparatorGenerator<?, S> comparatorGenerator) {
		AbstractLocalSearchAlgorithm<S> ret = newInstance(getProblem());
		ret.copyFor(this,s);
		if (comparatorGenerator != null) {
			ret.fitnessComparator = comparatorGenerator.generateComparator(getProblem());
		}
		ret.initialize();
		return ret;
	}
