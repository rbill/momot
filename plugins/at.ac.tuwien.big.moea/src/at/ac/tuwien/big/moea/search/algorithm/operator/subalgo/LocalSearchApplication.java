...
public class LocalSearchApplication<S extends Solution> extends AbstractProbabilityVariation implements ILocalSearchApplication {
	...
	public LocalSearchApplication(AbstractLocalSearchAlgorithm<S> baseAlgorithm, double probability, int maxSteps, USE_SOLUTION_TYPE solutionType,
	...
	public boolean continueSearch(AbstractLocalSearchAlgorithm<S> algorithmState, Solution curSolution, int curStep) {
		return curStep < maxSteps && !algorithmState.isTerminated();
	}
	...
	protected Solution[] doEvolve(Solution[] parents) {
		...
		int takeIndex = 0;
		IFitnessComparator<?, S> comparator = fitnessGenerator.generateComparator(algorithm.getProblem());
		if (tournamentSize > 1) {
