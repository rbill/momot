...
import at.ac.tuwien.big.momot.problem.unit.parameter.random.RandomBooleanValue;
...
public class ModuleManager {
	...
	protected void initUnits(Module module) {
		...
		for(EPackage ePackage : module.getImports())  {
			...
			EPackage existing = packageRegistry.getEPackage(uri);
			if (existing == null) {
				packageRegistry.putIfAbsent(uri, ePackage);
			}
		}
	...
	public<T> T nextParameterValueEarly(Parameter parameter, Assignment curValue) {
		...
		boolean mustBeSet = parameter.getKind() == ParameterKind.IN || parameter.getKind() == ParameterKind.INOUT;
		if (mustBeSet && value == null) {
			System.err.println("No parameter value for "+parameter+"!");
			assignSimpleParameterValue(parameter);
			value = parameterValues.get(parameter);
		}
	...
	private void assignSimpleParameterValue(Parameter parameter) {
		parameterValues.put(parameter, getDefaultUserParameterValue(parameter));
	}

	// convenience methods to get next parameter value
	public<T> T nextParameterValueLate(Parameter parameter, Assignment curValue) {
		IParameterValue value = parameterValues.get(parameter);
		boolean mustBeSet = false;
		if(value == null || (!mustBeSet  && !value.isLate()))
			return null;
		return (T)value.nextValue(curValue);
	}
	...
	public static IParameterValue<?> getDefaultUserParameterValue(Parameter parameter) {
		...
		if(clazz == Boolean.class)
			return new RandomBooleanValue(0.5);
