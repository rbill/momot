...
public class DiscardingSolutionInitializer<S extends TransformationSolution> implements ISolutionInitializer<S>{
	...
	private ParetoFront<S> curFront = new ParetoFront<S>();
	...
	public static class LinearQualityComparator implements Comparator<Solution> {
		...
		public Double getValue(Solution sol) {
			Double ret = calcValues.get(sol);
			if (ret == null) {
				calcValues.put(sol, ret= singleObjective.calculateSingleObjective(sol));
			}
			return ret;
		}
	...
	private IFitnessEvaluation<S> fitnessFunction;
	...
	public DiscardingSolutionInitializer(SearchHelper searchHelper, int generateSolutions, int solutionConsiderSize, int solutionUseSize,
			int solutionsPerQueue, int numQueues, int numInitSolutions, int sortEveryX, IFitnessEvaluation<S> fitnessFunction,
	...
	public void initWith(S baseSolution) {
		...
		curFront.tryAdd(baseSolution);
		this.maxQueueSize = Math.max(solutionsPerQueue*10,generateSolutions);
	...
	public PriorityQueue<S> createQueue(S baseSolution) {
		SingleObjectivizer objectivizer = provider.provide(curFront);
	...
	public void generateSolutions(S baseSolution, int solutionCount) {
		for (int i = 0; i < solutionCount; ++i) {
			...
			List<S> newSol =  helper.changeRandomVariables(curSolution, solutionConsiderSize, 0.1, 0.0, true, Collections.emptyList(), 0.7);
			for (S sol: newSol) {
	...
	public S getSolution(S baseSolution) {
		...
		if (returnSolutions.size() < generateSolutions && evaluationCount < maximumEvaluations) {
			generateSolutions(baseSolution,generateSolutions-returnSolutions.size());
		}
		if (returnSolutions.isEmpty()) {
			return (S)baseSolution.copy();
		}
		return returnSolutions.remove(0);
	}
	...
	public void setFitnessFunction( IFitnessEvaluation<S> fitnessFunction2) {
		this.fitnessFunction = fitnessFunction2;
	}
}
